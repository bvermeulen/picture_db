...
class PictureDb():
    ...
    def connect(cls):
        cls.connect_string = f'host=\'{cls.host}\' dbname=\'{cls.database}\''\
                             f'user=\'{cls.db_user}\' password=\'{cls.db_user_pw}\''

    def create_pictures_table(cls):
        with psycopg2.connect(cls.connect_string) as connection:
    ...
    def store_picture_meta(cls, filename):
        # file attributes
        ...
        with psycopg2.connect(cls.connect_string) as connection:
            ...
            try:
                ...
                print(sql_string)
                cur.execute(sql_string, (file_path, file_name, file_last_modified, file_created, file_size,
                                         date_picture, camera_make, camera_model,
...
def test():
    filename = './pics/IMG_2226.JPG'
    picdb = PictureDb()
    picdb.connect()
    picdb.store_picture_meta(filename)
    picdb.load_picture_meta(4)
